...
type getDepositRecordtructOutput = [BN, boolean, BN] & {
    principal: BN;
...
function checkDepositRecord(
    depositRecord: getDepositRecordtructOutput,
...
describe("TrancheVault Test", function () {
    ...
    describe("addApprovedLender", function () {
        ...
        it("Should allow pool operators to add a lender", async function () {
            ...
            let depositRecord = await juniorTrancheVaultContract.getDepositRecord(
    ...
    describe("removeApprovedLender", function () {
        ...
        it("Should allow a lender to be removed twice", async function () {
            ...
            expect(await juniorTrancheVaultContract.hasRole(lenderRole, lender4.address)).to.be
                .false;
            const newDepositRecord = await juniorTrancheVaultContract.getDepositRecord(
                lender4.address,
            );
        ...
        it("Should allow pool operators to remove a lender that was added first", async function () {
            ...
            const oldDepositRecord = await juniorTrancheVaultContract.getDepositRecord(
                defaultDeployer.address,
            );
    ...
    describe("setReinvestYield", function () {
        ...
        it("Should allow the reinvestYield option to be set to true if the lender's reinvestYield is false", async function () {
            ...
            expect(
                (await juniorTrancheVaultContract.getDepositRecord(protocolOwner.address))
                    .reinvestYield,
            ).to.equal(true);
        ...
        it("Should allow the reinvestYield option to be set to false if the lender's reinvestYield is true", async function () {
            ...
            expect(
                (await juniorTrancheVaultContract.getDepositRecord(defaultDeployer.address))
                    .reinvestYield,
            ).to.equal(false);
            let newNonReinvestingLendersLength =
                await juniorTrancheVaultContract.getNonReinvestingLendersLength();
    ...
    describe("Deposit Tests", function () {
        it("Should not allow deposits when the protocol is paused or the pool is off", async function () {
            await humaConfigContract.connect(protocolOwner).pause();
            await expect(
                juniorTrancheVaultContract.connect(lender).deposit(toToken(1)),
        ...
        it("Should not allow deposits with 0 amount", async function () {
            await expect(
                juniorTrancheVaultContract.connect(lender).deposit(0),
        ...
        it("Should not allow a non-Lender to deposit", async function () {
            await expect(
                juniorTrancheVaultContract.deposit(toToken(1)),
            ).to.be.revertedWithCustomError(juniorTrancheVaultContract, "LenderRequired");
        });
        ...
        it("Should now allow deposit amount less than the min requirement", async function () {
            ...
            await expect(
                juniorTrancheVaultContract
                    .connect(lender)
                    .deposit(poolSettings.minDepositAmount.sub(toToken(1))),
            ).to.be.revertedWithCustomError(juniorTrancheVaultContract, "DepositAmountTooLow");
        });
        ...
        it("Should not allow deposits that would result in the liquidity cap being exceeded", async function () {
            ...
            await expect(
                juniorTrancheVaultContract
                    .connect(lender)
                    .deposit(lpConfig.liquidityCap.add(BN.from(1))),
            ).to.be.revertedWithCustomError(
                juniorTrancheVaultContract,
        ...
        it("Should not allow deposits if the new senior total assets would exceed the maxSeniorJuniorRatio", async function () {
            ...
            await expect(
                seniorTrancheVaultContract.connect(lender).deposit(seniorDepositAmount),
            ).to.be.revertedWithCustomError(
                seniorTrancheVaultContract,
                "TrancheLiquidityCapExceeded",
            );
        ...
        it("Should allow lenders to deposit", async function () {
            ...
            await expect(juniorTrancheVaultContract.connect(lender).deposit(juniorAmount))
                .to.emit(juniorTrancheVaultContract, "LiquidityDeposited")
            ...
            let depositRecord = await juniorTrancheVaultContract.getDepositRecord(lender.address);
            ...
            depositRecord = await juniorTrancheVaultContract.getDepositRecord(lender.address);
            checkDepositRecord(
                depositRecord,
        ...
        describe("When there is PnL", function () {
            ...
            async function testDepositWithPnL(profit: BN, loss: BN, lossRecovery: BN) {
                ...
                await juniorTrancheVaultContract.connect(lender).deposit(juniorAmount);
                await seniorTrancheVaultContract.connect(lender2).deposit(seniorAmount);
                ...
                await setNextBlockTimestamp(ts);
                await expect(juniorTrancheVaultContract.connect(lender3).deposit(juniorAmount))
                    .to.emit(juniorTrancheVaultContract, "LiquidityDeposited")
                ...
                ts = ts + 3;
                await setNextBlockTimestamp(ts);
                await expect(seniorTrancheVaultContract.connect(lender4).deposit(seniorAmount))
                ...
                depositRecord = await seniorTrancheVaultContract.getDepositRecord(lender4.address);
    ...
    describe("Withdraw Tests", function () {
        ...
        describe("Redemption Tests", function () {
            describe("addRedemptionRequest", function () {
                ...
                it("Should allow lenders to request redemption in the same epoch", async function () {
                    ...
                    let principal = (
                        await juniorTrancheVaultContract.getDepositRecord(lender.address)
                    ).principal;
                    await expect(
                    ...
                    principal = (await juniorTrancheVaultContract.getDepositRecord(lender.address))
                        .principal;
                    await expect(
                        juniorTrancheVaultContract.connect(lender).addRedemptionRequest(shares),
                    )
                    ...
                    principal = (
                        await juniorTrancheVaultContract.getDepositRecord(lender2.address)
                    ).principal;
                    await expect(
                ...
                it("Should allow redemption requests from the pool owner treasury in the senior tranche w/o considering liquidity requirements", async function () {
                    ...
                    let principal = (
                        await seniorTrancheVaultContract.getDepositRecord(
                            poolOwnerTreasury.address,
                        )
                    ).principal;
                    await expect(
                ...
                it("Should allow redemption requests from the EA in the senior tranche w/o considering liquidity requirements", async function () {
                    ...
                    const sharesRequested = balance.sub(1);
                    let principal = (
                        await seniorTrancheVaultContract.getDepositRecord(evaluationAgent.address)
                    ).principal;
                    await expect(
                        seniorTrancheVaultContract
                            .connect(evaluationAgent)
            ...
            describe("cancelRedemptionRequest", function () {
                ...
                it("Should allow redemption request cancellation", async function () {
                    ...
                    let balance = await juniorTrancheVaultContract.balanceOf(lender.address);
                    let epochBefore =
                        await juniorTrancheVaultContract.getEpochRedemptionSummary(currentEpochId);
        ...
        describe("Disburse Tests", function () {
            ...
            it("Should disburse when one epoch was fully processed", async function () {
                ...
                let principal = (await seniorTrancheVaultContract.getDepositRecord(lender.address))
                    .principal;
                await expect(seniorTrancheVaultContract.connect(lender).disburse())
                    .to.emit(seniorTrancheVaultContract, "LenderFundDisbursed")
                    .withArgs(lender.address, shares);
                expect(await mockTokenContract.balanceOf(lender.address)).to.equal(
                    balanceBefore.add(shares),
                );
                expect(
                    (await seniorTrancheVaultContract.getDepositRecord(lender.address)).principal,
                ).to.equal(principal);
                await checkRedemptionRecordByLender(
                    seniorTrancheVaultContract,
                    lender,
                ...
                principal = (await seniorTrancheVaultContract.getDepositRecord(lender2.address))
                    .principal;
                await expect(seniorTrancheVaultContract.connect(lender2).disburse())
                    .to.emit(seniorTrancheVaultContract, "LenderFundDisbursed")
                    .withArgs(lender2.address, shares);
                expect(await mockTokenContract.balanceOf(lender2.address)).to.equal(
                    balanceBefore.add(shares),
                );
                expect(
                    (await seniorTrancheVaultContract.getDepositRecord(lender2.address)).principal,
                ).to.equal(principal);
                await checkRedemptionRecordByLender(
                    seniorTrancheVaultContract,
                    lender2,
            ...
            it("Should disburse when epochs were partially processed", async function () {
                ...
                await expect(seniorTrancheVaultContract.connect(lender).disburse())
                    .to.emit(seniorTrancheVaultContract, "LenderFundDisbursed")
                    .withArgs(lender.address, withdrawableAmount);
                expect(await mockTokenContract.balanceOf(lender.address)).to.equal(
                ...
                await expect(seniorTrancheVaultContract.connect(lender2).disburse())
                    .to.emit(seniorTrancheVaultContract, "LenderFundDisbursed")
                    .withArgs(lender2.address, withdrawableAmount2);
                expect(await mockTokenContract.balanceOf(lender2.address)).to.equal(
                ...
                let totalSharesRequested = (
                    await seniorTrancheVaultContract.getEpochRedemptionSummary(lastEpoch.id)
                ...
                await expect(seniorTrancheVaultContract.connect(lender).disburse())
                    .to.emit(seniorTrancheVaultContract, "LenderFundDisbursed")
                    .withArgs(lender.address, allShares.sub(2));
                ...
                await expect(seniorTrancheVaultContract.connect(lender2).disburse())
                    .to.emit(seniorTrancheVaultContract, "LenderFundDisbursed")
                    .withArgs(lender2.address, allShares2.sub(2));
            ...
            describe("When there is PnL", function () {
                it("Should disburse when there is profit", async function () {
                    ...
                    await expect(juniorTrancheVaultContract.connect(lender).disburse())
                        ...
                        .withArgs(lender.address, amountProcessed);
                    expect(await mockTokenContract.balanceOf(lender.address)).to.equal(
    ...
    describe("processYieldForLenders", function () {
        ...
        async function prepareForYieldTests(lenderAddrs: Signer[]): Promise<Lender[]> {
            ...
            for (let i = 0; i < lenderAddrs.length; i++) {
                ...
                await juniorTrancheVaultContract.connect(lender.lender).deposit(amount);
                ...
                await seniorTrancheVaultContract.connect(lender.lender).deposit(amount);
            }
        ...
        it("Should reinvest yields", async function () {
            ...
            expect(
                (await juniorTrancheVaultContract.getDepositRecord(lender.address)).principal,
            ).to.be.closeTo(lenders[0].principal, 1);
            expect(
                (await juniorTrancheVaultContract.getDepositRecord(lender2.address)).principal,
            ).to.be.closeTo(lenders[1].principal, 1);
            expect(
                await poolSafeContract.unprocessedTrancheProfit(
                    juniorTrancheVaultContract.address,
        ...
        it("Should payout yields and reinvest yields", async function () {
            ...
            expect(
                (await juniorTrancheVaultContract.getDepositRecord(lender3.address)).principal,
            ).to.be.closeTo(lenders[2].principal, 1);
            expect(
                (await juniorTrancheVaultContract.getDepositRecord(lender4.address)).principal,
        ...
        it("Should do nothing when there is loss", async function () {
            ...
            expect(await mockTokenContract.balanceOf(lender4.address)).to.equal(lender4Assets);
            expect(
                (await juniorTrancheVaultContract.getDepositRecord(lender.address))[0],
            ).to.equal(lenders[0].principal);
            expect(
                (await juniorTrancheVaultContract.getDepositRecord(lender2.address))[0],
            ).to.equal(lenders[1].principal);
            expect(
                (await juniorTrancheVaultContract.getDepositRecord(lender3.address))[0],
            ).to.equal(lenders[2].principal);
            expect(
                (await juniorTrancheVaultContract.getDepositRecord(lender4.address))[0],
